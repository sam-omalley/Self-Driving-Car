...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        new_y = 0
        new_theta = 0
        steer = random.gauss(motion[0], self.steering_noise)
        d =  random.gauss(motion[1], self.distance_noise)
        B = (float(motion[1]) / float(self.length)) * tan(float(steer))
        if abs(B) < 0.001:
            new_x = self.x + d * cos(self.orientation)
            new_y = self.y + d * sin(self.orientation)
            new_theta = (self.orientation + B) % (2*pi)
        else:
            R = motion[1] / B
    ...
    # sense: 
    #    
    def sense(self, opt = 1): #do not change the name of this function
        if opt == 0:
            beari_noise = 0
        else:
            beari_noise = bearing_noise
        Z = []

        for mark in landmarks:
            dy = mark[0] - self.y
            dx = mark[1] - self.x
            Z.append((atan2(dy, dx)%(2*pi)) - (self.orientation + random.gauss(0.0, beari_noise)))
        # ENTER CODE HERE
